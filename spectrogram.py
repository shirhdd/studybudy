...
def plot_audio_spectrogram(audio_path, binsize=2**10, plot_path=None, argv = '', colormap="jet"):
    ...
    #plt.plot(new_spectrogram[0],freq)
    freq_new = []
    spect_new = []
    for index,i in enumerate(freq):
        if i > 2000 and i < 10000:
            freq_new.append(i)
            spect_new.append(new_spectrogram[5][index])

    #print()
    #print(start_from))
    plt.plot(freq_new, spect_new)

    # plt.plot(freq,new_spectrogram[2])
    # plt.plot(freq,new_spectrogram[3])
    # plt.plot(freq,new_spectrogram[4])
